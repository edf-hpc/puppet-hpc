...
#### Fact: `my_XXXXX_{server,relay,mirror}` (profiles)

Those facts give for each profile ``profiles::XXXXXX::{server,relay,mirror}`` which role has the profile defined. This value can then be used elsewhere in the configuration to configure the clients.
