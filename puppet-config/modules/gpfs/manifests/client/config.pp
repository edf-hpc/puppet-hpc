...
class gpfs::client::config inherits gpfs::client {
  ...
  $gpfs_cl_files = {
    "${gpfs::client::cl_config}" => {
      content  => decrypt($gpfs::client::cl_config_src, $gpfs::client::decrypt_passwd),
      mode     => $gpfs::client::file_mode,
    },
    "${gpfs::client::cl_key}" => {
      content  => decrypt($gpfs::client::cl_key_src, $gpfs::client::decrypt_passwd),
    },
    "${gpfs::client::cl_perf}" => {
      source   => $gpfs::client::cl_perf_src,
      ensure   => 'directory',
      mode     => $gpfs::client::dir_mode,
      recurse  => 'remote',
    },
  }
